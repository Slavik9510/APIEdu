...
export class MessageService {
  ...
  createHubConnection(user: User, otherUsername: string) {
    ...
    this.hubConection.on('UpdatedGroup', (group: Group) => {
    ...
    this.hubConection.on('NewMessage', message => {
      this.messageThread$.pipe(take(1)).subscribe({
        next: messages => {
  ...
  async sendMessage(username: string, content: string) {
    return this.hubConection?.invoke('SendMessageAsync', { recepientUsername: username, content })
